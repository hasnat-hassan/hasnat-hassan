<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:6D28D9,50:7C3AED,100:4F46E5&height=220&section=header&text=Hasnat%20Hassan&fontSize=52&fontColor=ffffff&animation=fadeIn&fontAlignY=38&desc=Frontend%20Developer%20%7C%20React.js%20%E2%80%A2%20Next.js%20%E2%80%A2%20TypeScript&descAlignY=58&descSize=18" width="100%"/>

<a href="https://git.io/typing-svg">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=24&duration=3000&pause=1000&color=A78BFA&center=true&vCenter=true&width=650&lines=Frontend+Developer+%40+Revnix;React.js+%7C+Next.js+%7C+TypeScript+Engineer;Building+Scalable%2C+Production-Ready+Web+Apps;9%2B+Products+Shipped+to+Production" alt="Typing SVG" />
</a>

<br/>

![Diploma](https://img.shields.io/badge/DAE-Info%20%26%20Comm.%20Technology-6D28D9?style=flat-square&labelColor=1a1a2e)
![BS Software Engineering](https://img.shields.io/badge/BS-Software%20Engineering%20(In%20Progress)-7C3AED?style=flat-square&labelColor=1a1a2e)
![Location](https://img.shields.io/badge/📍-Haripur,%20Khyber%20Pakhtunkhwa,%20Pakistan-4F46E5?style=flat-square&labelColor=1a1a2e)

<br/>

[![Portfolio](https://img.shields.io/badge/Portfolio-hasnat--hassan.me-8B5CF6?style=for-the-badge&logo=vercel&logoColor=white&labelColor=1a1a2e)](https://hasnat-hassan.me)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-6D28D9?style=for-the-badge&logo=linkedin&logoColor=white&labelColor=1a1a2e)](https://linkedin.com/in/hasnat-hassan)
[![Email](https://img.shields.io/badge/Email-Say%20Hello-7C3AED?style=for-the-badge&logo=gmail&logoColor=white&labelColor=1a1a2e)](mailto:hasnathassan397@gmail.com)
[![GitHub](https://img.shields.io/badge/GitHub-Follow-4F46E5?style=for-the-badge&logo=github&logoColor=white&labelColor=1a1a2e)](https://github.com/hasnat-hassan)

<br/>

![Profile Views](https://komarev.com/ghpvc/?username=hasnat-hassan&color=7c3aed&style=flat-square&label=Profile+Views)
![Followers](https://img.shields.io/github/followers/hasnat-hassan?color=6D28D9&style=flat-square&label=Followers&labelColor=1a1a2e)
![Stars](https://img.shields.io/github/stars/hasnat-hassan?color=4F46E5&style=flat-square&label=Stars&labelColor=1a1a2e)

</div>

<br/>

---

## 🧬 About Me

```txt
const hasnat = {
    role: "Frontend Developer",
    location: "Haripur, Khyber Pakhtunkhwa, Pakistan",
    experience: "2+ years in production environments",
    stack: ["React.js", "Next.js", "TypeScript"],
    focus: "Scalable, accessible, production-ready web applications",
    currentlyAt: "Revnix",
    education: "BS Software Engineering @ Virtual University of Pakistan (in progress)"
};
```

I'm a Frontend Developer with **2+ years of experience** building and shipping production web applications using **React.js, Next.js, and TypeScript**. My work spans across **9+ production products** — from client-facing platforms to internal tools — with a strong emphasis on component architecture, performance optimization, and clean, maintainable code.

Beyond the frontend, I regularly work across the stack: integrating REST APIs, managing **PostgreSQL** and **MongoDB** databases with **Drizzle ORM**, and handling deployments through **Docker, GitHub Actions, Vercel, and Coolify**. I also conduct code reviews and mentor junior developers and interns, which keeps me equally focused on code quality and team growth.

<table>
<tr>
<td>

**🎯 Open To**
- Frontend / Full-Stack Engineering roles
- Freelance & contract-based product work
- Collaborating on open-source tooling
- Mentorship and code review opportunities

</td>
</tr>
</table>

<br/>

---

## 🛠️ Tech Stack

**Languages**

<img src="https://skillicons.dev/icons?i=js,ts,html,css&theme=dark" />

**Frontend**

<img src="https://skillicons.dev/icons?i=react,nextjs,tailwind,materialui&theme=dark" />
<br/>
<img src="https://img.shields.io/badge/shadcn%2Fui-000000?style=flat-square&logo=shadcnui&logoColor=white&labelColor=1a1a2e" />
<img src="https://img.shields.io/badge/React_Hook_Form-EC5990?style=flat-square&logo=reacthookform&logoColor=white&labelColor=1a1a2e" />
<img src="https://img.shields.io/badge/Zod-3E67B1?style=flat-square&logo=zod&logoColor=white&labelColor=1a1a2e" />
<img src="https://img.shields.io/badge/Redux-764ABC?style=flat-square&logo=redux&logoColor=white&labelColor=1a1a2e" />
<img src="https://img.shields.io/badge/Zustand-433E38?style=flat-square&labelColor=1a1a2e" />
<img src="https://img.shields.io/badge/TanStack_Query-FF4154?style=flat-square&logo=reactquery&logoColor=white&labelColor=1a1a2e" />

**Backend & Databases**

<img src="https://skillicons.dev/icons?i=nodejs,postgres,mongodb,supabase&theme=dark" />
<br/>
<img src="https://img.shields.io/badge/Drizzle_ORM-C5F74F?style=flat-square&logoColor=black&labelColor=1a1a2e" />
<img src="https://img.shields.io/badge/NextAuth%2FAuth.js-7C3AED?style=flat-square&labelColor=1a1a2e" />
<img src="https://img.shields.io/badge/REST_APIs-4F46E5?style=flat-square&labelColor=1a1a2e" />

**Cloud, DevOps & Tooling**

<img src="https://skillicons.dev/icons?i=docker,git,github,figma,vscode,postman,vercel&theme=dark" />
<br/>
<img src="https://img.shields.io/badge/GitHub_Actions-2088FF?style=flat-square&logo=githubactions&logoColor=white&labelColor=1a1a2e" />
<img src="https://img.shields.io/badge/Coolify-6D28D9?style=flat-square&labelColor=1a1a2e" />
<img src="https://img.shields.io/badge/Cursor_IDE-000000?style=flat-square&labelColor=1a1a2e" />

<br/>

---

## 🤖 AI-Assisted Engineering

> A note on this section: my CV doesn't list formal AI/ML engineering experience, so rather than overstate it, here's an honest snapshot of how AI tooling fits into my actual workflow.

| Area | How I Use It |
|---|---|
| **AI Pair Programming** | Use Claude and Cursor IDE daily for code review, refactoring, and accelerating implementation |
| **Workflow Integration** | Incorporate AI-assisted tooling into day-to-day frontend development at Revnix |
| **Tooling Projects** | Built [`JSON to TOON`](#-featured-projects), a data-format conversion utility, as part of the internal product suite |

<br/>

---

## 🚀 Featured Projects

<details>
<summary><b>🏗️ 21stCenturyEquipment</b></summary>
<br/>

Client platform built with reusable React.js/Next.js components and REST API integrations, with a focus on improving responsive performance across devices.

| | |
|---|---|
| **Stack** | React.js, Next.js, TypeScript, REST APIs |
| **Category** | Client Platform |
| **Focus** | Reusable components, responsive performance |
| **Repository** | Private / Client Project |

</details>

<details>
<summary><b>🏗️ 4RiversEquipment</b></summary>
<br/>

Responsive, mobile-first client platform with optimized content rendering and production-grade feature delivery.

| | |
|---|---|
| **Stack** | React.js, Next.js, TypeScript |
| **Category** | Client Platform |
| **Focus** | Mobile-first design, content rendering optimization |
| **Repository** | Private / Client Project |

</details>

<details>
<summary><b>🏗️ TheBackyard</b></summary>
<br/>

Accessible, performance-optimized frontend built directly from Figma designs for a production client.

| | |
|---|---|
| **Stack** | React.js, Next.js, TypeScript |
| **Category** | Client Platform |
| **Focus** | Accessibility, design-to-code fidelity, performance |
| **Repository** | Private / Client Project |

</details>

<details>
<summary><b>⚙️ Nextly CMS</b></summary>
<br/>

Role-based internal CMS built with Admin, Editor, and Viewer permission tiers. Includes a custom-built CLI tool with token-based authentication, and integrates across multiple production websites.

| | |
|---|---|
| **Stack** | React.js, Next.js, TypeScript, Node.js, PostgreSQL |
| **Category** | Internal Product |
| **Focus** | Role-based access control, CLI tooling, multi-site integration |
| **Repository** | Private / Internal Project |

</details>

<details>
<summary><b>⚙️ Rext.ai · NextlyHQ · CodexSpot · JSON to TOON</b></summary>
<br/>

Suite of internal products where frontend features were developed alongside REST API integration, PostgreSQL database management via Drizzle ORM, and Vercel/Coolify deployment configuration and production issue resolution.

| | |
|---|---|
| **Stack** | React.js, Next.js, TypeScript, PostgreSQL, Drizzle ORM |
| **Category** | Internal Product Suite |
| **Focus** | Full-stack feature development, deployment configuration |
| **Repository** | Private / Internal Project |

</details>

<br/>

---

## 💼 Experience

### Frontend Developer
**Revnix — Haripur, Pakistan** &nbsp;|&nbsp; *August 2024 – Present*

Deliver frontend development across 9+ production web applications, including client platforms and internal products, using React.js, Next.js, and TypeScript.

- Built reusable component libraries and implemented responsive interfaces from Figma designs
- Improved application performance through lazy loading, code splitting, dynamic imports, and rendering optimizations
- Integrated REST APIs and worked with Supabase, PostgreSQL, MongoDB, and Drizzle ORM for backend integration and schema migrations
- Contributed to Nextly CMS, implementing role-based content management and supporting production deployments via Docker, GitHub Actions, Vercel, and Coolify
- Conducted code reviews, mentored junior developers and interns, and collaborated cross-functionally with designers and backend engineers

`React.js` `Next.js` `TypeScript` `PostgreSQL` `Drizzle ORM` `Docker` `Vercel`

<br/>

### Frontend Development Intern
**Revnix — Haripur, Pakistan** &nbsp;|&nbsp; *February 2024 – July 2024*

- Contributed to production React.js and Next.js applications by building UI components, integrating features, and fixing bugs
- Collaborated through Git workflows, leading to promotion to Frontend Developer

`React.js` `Next.js` `Git` `UI Development`

<br/>

---

## 🏆 Achievements

<div align="center">

| Recognition | Details |
|---|---|
| *Add your achievement* | *Add details here* |

</div>

> *No achievements were listed on your CV — replace the row above, or let me know and I'll remove this section.*

<br/>

---

## 📜 Certifications & Courses

<div align="center">

### Udemy

[![HTML5 & CSS3](https://img.shields.io/badge/Design%20%26%20Develop%20a%20Killer%20Website%20with%20HTML5%20%26%20CSS3-6D28D9?style=for-the-badge&logo=udemy&logoColor=white&labelColor=1a1a2e)](https://www.udemy.com/course/design-and-develop-a-killer-website-with-html5-and-css3/)

[![Modern JavaScript](https://img.shields.io/badge/Modern%20JavaScript%20From%20The%20Beginning-7C3AED?style=for-the-badge&logo=udemy&logoColor=white&labelColor=1a1a2e)](https://www.udemy.com/course/modern-javascript-from-the-beginning/)

[![The Ultimate React Course](https://img.shields.io/badge/The%20Ultimate%20React%20Course-4F46E5?style=for-the-badge&logo=udemy&logoColor=white&labelColor=1a1a2e)](https://www.udemy.com/course/the-ultimate-react-course/)

[![Next.js Full Stack](https://img.shields.io/badge/Next.js%20%E2%80%94%20Build%20Full%20Stack%20Apps%20with%20Next.js%20%26%20TypeScript-8B5CF6?style=for-the-badge&logo=udemy&logoColor=white&labelColor=1a1a2e)](https://www.udemy.com/course/nextjs-build-full-stack-apps-with-nextjs-using-redux/)

</div>

<br/>

---

## 💻 Coding Profiles

<div align="center">

*No coding profile links (LeetCode, GeeksforGeeks, HackerRank, CodeChef) were on your CV — send me the usernames and I'll add premium badges here.*

</div>

<br/>

---

## 📊 GitHub Analytics

<div align="center">

<img src="https://github-readme-stats.vercel.app/api?username=hasnat-hassan&show_icons=true&theme=tokyonight&hide_border=true&bg_color=0D1117&title_color=A78BFA&icon_color=8B5CF6&text_color=C9D1D9" width="49%" />
<img src="https://github-readme-streak-stats.herokuapp.com/?user=hasnat-hassan&theme=tokyonight&hide_border=true&background=0D1117&stroke=7C3AED&ring=8B5CF6&fire=A78BFA&currStreakLabel=A78BFA" width="49%" />

<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=hasnat-hassan&layout=compact&theme=tokyonight&hide_border=true&bg_color=0D1117&title_color=A78BFA&text_color=C9D1D9" width="49%" />

</div>

<br/>

---

## 🏅 GitHub Trophies

<div align="center">

<img src="https://github-profile-trophy.vercel.app/?username=hasnat-hassan&theme=discord&no-frame=true&row=1&column=6&margin-w=8" />

</div>

<br/>

---

## 📈 Contribution Activity

<div align="center">

<img src="https://github-readme-activity-graph.vercel.app/graph?username=hasnat-hassan&theme=tokyo-night&hide_border=true&bg_color=0D1117&color=A78BFA&line=7C3AED&point=8B5CF6" width="100%" />

</div>

<br/>

---

## 🐍 Contribution Snake

<div align="center">

<img src="https://raw.githubusercontent.com/hasnat-hassan/hasnat-hassan/output/github-contribution-grid-snake-dark.svg" width="100%" />

<sub>Generated via <a href="https://github.com/Platane/snk">Platane/snk</a> — requires a scheduled GitHub Action in your profile repo to render.</sub>

</div>

<br/>

---

## 🎯 Current Focus

```yaml
Learning:
  - Software Engineering fundamentals (BS @ Virtual University of Pakistan)
  - Deeper backend architecture & database design patterns

Building:
  - Production-ready frontend systems across 9+ live products @ Revnix
  - Internal tooling (Nextly CMS, JSON to TOON, and related internal products)

Exploring:
  - AI-assisted development workflows (Claude, Cursor IDE)
  - Full-stack ownership from Figma to deployment

Open To:
  - Frontend / Full-Stack Engineering opportunities
  - Collaborative and open-source projects
```

<br/>

---

## 📬 Connect With Me

<div align="center">

[![Gmail](https://img.shields.io/badge/Gmail-hasnathassan397%40gmail.com-6D28D9?style=for-the-badge&logo=gmail&logoColor=white&labelColor=1a1a2e)](mailto:hasnathassan397@gmail.com)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-hasnat--hassan-7C3AED?style=for-the-badge&logo=linkedin&logoColor=white&labelColor=1a1a2e)](https://linkedin.com/in/hasnat-hassan)
[![GitHub](https://img.shields.io/badge/GitHub-hasnat--hassan-4F46E5?style=for-the-badge&logo=github&logoColor=white&labelColor=1a1a2e)](https://github.com/hasnat-hassan)
[![Portfolio](https://img.shields.io/badge/Portfolio-hasnat--hassan.me-8B5CF6?style=for-the-badge&logo=vercel&logoColor=white&labelColor=1a1a2e)](https://hasnat-hassan.me)

</div>

<br/>

---

<div align="center">

*"Clean code, thoughtful design, and reliable systems — built one production release at a time."*

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:4F46E5,50:7C3AED,100:6D28D9&height=120&section=footer" width="100%"/>

</div>
